...
/**
 * Tests out a case where the access token is munged (prefixed with "_").  In this case, a single request
 * fails but all other requests proceed normally after a refresh token exchange to get a new access token.
 *
 * The single request must fail because it's entity (request payload) may have already been streamed and so
 * it cannot be reused.  If we wanted to work around this, we'd have to somehow cache the incoming stream ahead of
 * sending it and that's too expensive in terms of both memory and disk space.
 *
 * @author uzi
 */
public class RefreshToken1Test extends TestCase
{
    @Test
    public void test()
        throws Exception
    ...
    public class RefreshTokenTestSabotageRunner extends Thread
    {
        ...
        public void run()
        {
            ...
            while (!terminate)
            {
                ...
                executor.acquireTokenLock();
                try
                {
                    String accessToken = executor.getState().getAccessToken();
                    executor.getState().setAccessToken("_" + accessToken.substring(1));
                }
                finally
                {
                    executor.releaseTokenLock();
                }
            }
        }
    }
