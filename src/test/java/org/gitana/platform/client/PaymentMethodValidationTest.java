...
public class PaymentMethodValidationTest extends AbstractTestCase
{
        ...
        throws Exception
    {
        ...
        // validate (GOOD)
        PaymentMethodValidation validation1 = registrar.validateCreditCard("Chuck Berry", NUMBER_VISA_GOOD, 1, 2020);
        assertTrue(validation1.isValid());
        assertTrue(validation1.listErrors().size() == 0);

        // validate (BAD NUMBER)
        PaymentMethodValidation validation2 = registrar.validateCreditCard("Chuck Berry", NUMBER_VISA_BAD, 1, 2020);
        assertFalse(validation2.isValid());
        assertTrue(validation2.listErrors().size() > 0);

        // validate (BAD MONTH)
        PaymentMethodValidation validation3 = registrar.validateCreditCard("Chuck Berry", NUMBER_VISA_GOOD, 13, 2020);
